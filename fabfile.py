...
def ows_test():
    ...
    env.static_dirs = '/var/www/html/static/%(sitename)s' % env
...
def deploy():
    ...
    """ 
    init_deploy_procedure()
    copy_working_dir()
    custom_procedure()
    bounce_apache()

def init_deploy_procedure():
    local('scp %(host)s:%(path)s/%(db_name)s .' % env)
...
def custom_procedure():
    #Static Files
    sudo('mkdir -p %(static_dirs)s' % env)
    sudo('cd %(path)s;cp -r static/%(sitename)s/* %(static_dirs)s' % env)
    #Sqlite database permission
    #print 'chown -R mstacy:ows %(path)s; chmod -r 775 %(path)s;' % env
    sudo('chown -R mstacy:ows %(path)s; chmod -R 775 %(path)s;' % env)
